...
impl RateLimiter {
    ...
    pub fn verify_packet<'a, 'b>(
        ...
        | Packet::HandshakeResponse(HandshakeResponse { sender_idx, .. }) = packet
        {
            ...
            Option::from(subtle::ConstantTimeEq::ct_eq(&computed_mac1[..16], mac1))
                .ok_or(TunnResult::Err(WireGuardError::InvalidMac))?;
            ...
            if self.is_under_load() {
                ...
                if subtle::ConstantTimeEq::ct_eq(&computed_mac2[..16], mac2).into() {
